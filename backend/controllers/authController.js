...
// @desc    Ward Admin Login
// @route   POST /api/auth/wardadmin/login
// @access  Public
exports.wardAdminLogin = async (req, res, next) => {
  try {
    ...
    // Check if ward admin exists
    const wardAdmin = await User.findOne({
      role: 'wardAdmin',
      email: username, // assuming you're storing username as email
      ward: wardNumber
    }).select('+password');
    ...
    if (!wardAdmin) {
      return res.status(401).json({
        success: false,
        error: 'Ward Admin not found for this ward'
      });
    }

    const isMatch = await wardAdmin.comparePassword(password);
